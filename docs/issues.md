...
- **상태:** `main` 병합 완료. 실사용 사이트 테스트 결과는 아래 [[#엣지-케이스-점검-2026-09-04-claude-in-chrome로-실사용-테스트]] 참고.
...
## 엣지 케이스 점검 (2026-09-04, claude-in-chrome로 실사용 테스트)
- [x] YouTube 동영상 컨트롤 (재생바 탐색, 볼륨 슬라이더 드래그) — 정상
- [x] Google Docs 단축키 (Ctrl+A, Ctrl+S) — 정상, Ctrl+S에서 네이티브 저장 다이얼로그 없이 자체 저장 처리 확인 (이번 수정의 핵심 목적)
- [x] ChatGPT 답변 복사 버튼 — 정상
- [ ] Bilibili 플레이어 컨트롤 — 미확인 (영상 로드 실패, 지역 제한 추정). 애초 경쟁사 이슈는 오버레이 기반 확장 얘기라 제로 DOM 조작 구조인 우리와는 리스크 성격이 다름
- [ ] Overleaf Ctrl+S/Ctrl+Enter — 미확인 (브라우저에 로그인 안 되어 있어 스킵, 대리 로그인 안 함)
- [ ] Instagram — 미확인 (계정 보안 확인 화면 떠서 중단 — 사용자가 직접 처리 필요)
- [ ] Spotify — 미확인 (쿠키 배너까지만 확인, 시간 관계상 중단)
