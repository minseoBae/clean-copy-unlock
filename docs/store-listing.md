...
## 광범위한 호스트 권한 경고 (심사 관련)
스토어 대시보드가 `<all_urls>` 콘텐츠 스크립트를 "광범위한 호스트 권한"으로 분류해 심층 검토(수동 리뷰) 대상이 됨을 경고함. `activeTab`으로 바꾸면 심사는 빨라지지만 "사이트마다 아이콘을 클릭해야만 동작"하는 방식으로 바뀌어 이 확장의 핵심 가치(자동으로 항상 풀려있음)를 잃음. **`<all_urls>` 유지로 결정**(2026-09-04, 사용자 확인) — 심사가 오래 걸리는 건 감수.
